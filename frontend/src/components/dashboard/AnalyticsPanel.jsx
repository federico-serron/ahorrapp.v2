...
const buildCumulative = (byDate) => {
  let running = 0;
  return byDate.map(({ date, expenses }) => {
    running += expenses;
    return { day: date.slice(8), total: running };
  });
};
...
const CumulativeTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 shadow-lg text-xs">
      <p className="font-medium text-gray-500 dark:text-gray-400 mb-1">Día {label}</p>
...
export default function AnalyticsPanel() {
  ...
  // Filtro del gráfico de línea (solo mes)
  const [lineMonth, setLineMonth] = useState(currentYearMonth());

  // Filtro del pie chart (mes o rango)
  ...
  const handleApplyLine = () => {
    const { start, end } = monthToRange(lineMonth);
    actions.getLineAnalytics(start, end);
  };

  const handleApplyPie = () => {
    if (filterMode === 'month') {
      const { start, end } = monthToRange(selectedMonth);
      actions.getAnalytics(start, end);
    } else {
      if (rangeStart && rangeEnd) actions.getAnalytics(rangeStart, rangeEnd);
    }
  };

  const { analytics, analytics_loaded, line_analytics, line_analytics_loaded } = store;

  const cumulativeData = line_analytics_loaded && line_analytics
    ? buildCumulative(line_analytics.by_date)
    : [];
  ...
  return (
    <div className="space-y-10">
      ...
      {/* ── Gráfico de línea: gasto acumulado del mes ── */}
      <div className="space-y-4">
        ...
        {!line_analytics_loaded ? (
          ...
          <p className="text-sm text-gray-400 dark:text-gray-600 py-8 text-center">Sin gastos en este mes</p>
        ) : (
      ...
      {/* ── Filtro y summary para pie chart ── */}
      <div className="space-y-4">
